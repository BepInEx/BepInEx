	...
	public static class UnityInput
	{
		...
		public static IInputSystem Current
		{
			get
			{
				if (_current == null)
				{
                    //todo needs testing, only needed in il2cpp? or also in new unity mono versions?
                    AppDomain.CurrentDomain.TypeResolve += TypeResolveHandler;

					_current = FindInputSystem();

					// bug? if the resolve handler is needed, do not remove it?
					AppDomain.CurrentDomain.TypeResolve -= TypeResolveHandler;
				}
		...
		private static Assembly TypeResolveHandler(object sender, ResolveEventArgs args)
		{
			// UnityEngine.Input is usually in UnityEngine.CoreModule but in newer games can be in UnityEngine.InputLegacyModule with no type forwarding
			// in this case we need to make it load from the legacy assembly manually
			Console.WriteLine("resolve " + args.Name);
			if (args.Name == "UnityEngine.Input")
			{
				return Assembly.Load("UnityEngine.InputLegacyModule");
			}
		...
		private static IInputSystem FindInputSystem()
		{
			IInputSystem current;
            try
            {
				try
				{
					current = new LegacyInputSystem();
					current.GetKey(KeyCode.A);
