...
namespace BepInEx
{
	...
	internal class NewInputSystem : IInputSystem
	{
		...
		{
			return GetControl(name)?.isPressed ?? false;
		}
		...
		{
			return GetControl(name)?.wasPressedThisFrame ?? false;
		}
		...
		{
			return GetControl(name)?.wasReleasedThisFrame ?? false;
		}
		...
		{
			return GetControl(KeyCode.Mouse0 + button)?.isPressed ?? false;
		}

		public bool GetMouseButtonDown(int button)
		{
			return GetControl(KeyCode.Mouse0 + button)?.wasPressedThisFrame ?? false;
		}

		public bool GetMouseButtonUp(int button)
		{
			return GetControl(KeyCode.Mouse0 + button)?.wasReleasedThisFrame ?? false;
		}
		...
		private static ButtonControl GetControl(string name)
		{
			return GetControl((KeyCode)Enum.Parse(typeof(KeyCode), name, true));
		}
		...
		private static ButtonControl GetControl(KeyCode key, bool silent = false)
		{
			...
			if (!silent)
				Logger.LogDebug($"[{nameof(NewInputSystem)}] Unsupported key: {key}");
