	...
	public unsafe class IL2CPPDetourMethodPatcher : MethodPatcher
	{
		...
		private void Init()
		{
			try
			{
				...
				if (methodField == null)
				{
					var fieldInfoField = UnhollowerUtils.GetIl2CppFieldInfoPointerFieldForGeneratedFieldAccessor(Original);

					if (fieldInfoField != null)
						throw new Exception($"Method {Original.FullDescription()} is a field accessor, it can't be patched.");
		...
		private DynamicMethodDefinition GenerateNativeToManagedTrampoline(MethodInfo targetManagedMethodInfo)
		{
			...
			int paramStartIndex = Original.IsStatic ? 0 : 1;
